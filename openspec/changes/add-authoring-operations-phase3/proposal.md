...
現行のMCPは単発の staged 追加には十分だが、教材を継続的に投入・整理する authoring 基盤としては不足がある。問題データを安全に増やす前に、batch 操作、draft 詳細参照、custom card type 定義の運用整理、そしてそれらの最小テスト契約を固める必要がある。

## What Changes

- batch で staged draft を作成・commit・discard できる MCP ツール契約を追加する
- staged draft 1件の詳細を取得する read ツール契約を追加する
...
### New Capabilities
- `authoring-batch-operations`: batch staged create/commit/discard の契約、制約、部分失敗ポリシーを定義する
- `staged-draft-inspection`: staged draft 1件の詳細取得と agent-side review 用の安定レスポンスを定義する
