...
- [x] 1.1 Add request/response schemas and contract resource entries for `get_staged_card`, `list_card_type_definitions`, `deprecate_card_type_definition`, `create_staged_cards_batch`, `commit_staged_cards_batch`, and `discard_staged_cards_batch`
...
- [x] 2.1 Implement `get_staged_card` service and MCP handler with profile-scoped access checks
- [x] 2.2 Implement `list_card_type_definitions` and `deprecate_card_type_definition` service and MCP handlers
- [x] 2.3 Implement `create_staged_cards_batch` using per-item execution and stable itemized responses
- [x] 2.4 Implement `commit_staged_cards_batch` and `discard_staged_cards_batch` using existing single-item semantics per item
- [x] 2.5 Reject deprecated custom card type definitions in new staged-card creation while preserving read access
...
- [x] 3.3 Extend real-Anki smoke coverage with one batch staged-create path and one batch finalize path that clean up after execution
