...
The MCP server MUST implement card creation as a two-phase workflow (`staged` then `committed`) so users can inspect rendering in Anki GUI before finalizing.

#### Scenario: Create staged card successfully
- **WHEN** the client calls `create_staged_card` with valid card type and fields
- **THEN** the server creates a draft note tagged as staged and returns a `draftId` with linked `noteId` and `cardIds`

#### Scenario: Staged card metadata is persisted
- **WHEN** a staged card is created
...
- **WHEN** a staged card is created under an active Anki profile
...
- **WHEN** `create_staged_card` creates a draft
- **THEN** the server applies staged-only markers and excludes generated cards from normal study flow until commit
...
- **THEN** only `staged -> committed`, `staged -> discarded`, `staged -> superseded`, and `superseded -> discarded` transitions are allowed

#### Scenario: Invalid transitions are blocked
- **WHEN** a client attempts `committed -> staged` or `discarded -> committed`
- **THEN** the server returns `INVALID_STATE_TRANSITION` and performs no mutation

#### Scenario: Commit is idempotent
- **WHEN** `commit_staged_card` is called repeatedly for the same already committed draft
...
The MCP server MUST provide idempotent `create_staged_card` behavior using a client-supplied request key.

#### Scenario: Missing idempotency key is rejected
- **WHEN** `create_staged_card` is called without `clientRequestId`
- **THEN** the server returns `INVALID_ARGUMENT` and performs no mutation

#### Scenario: Retry with same key returns same draft
- **WHEN** `create_staged_card` is retried with the same `profileId` and `clientRequestId`
...
#### Scenario: Open staged card in browser
- **WHEN** the client calls `open_staged_card_preview` for an existing draft
- **THEN** the server opens or focuses Anki Browser with the staged note selected

#### Scenario: Preview command fails gracefully when GUI unavailable
- **WHEN** Anki GUI is not available during `open_staged_card_preview`
- **THEN** the server returns a structured recoverable error with guidance and keeps staged data intact
...
- **WHEN** the client calls `commit_staged_card`
...
- **WHEN** `commit_staged_card` omits the review decision payload
- **THEN** the server returns `INVALID_ARGUMENT` and performs no mutation

### Requirement: Explicit Commit and Discard Operations
The MCP server MUST separate `commit` and `discard` into dedicated tools and MUST NOT auto-commit staged cards.

#### Scenario: Commit staged card
- **WHEN** the client calls `commit_staged_card` for a valid draft
- **THEN** the server removes staged markers, applies final deck/tags if provided, and marks lifecycle state as `committed`

#### Scenario: Commit releases study isolation
- **WHEN** a staged draft is committed
- **THEN** the server removes staged-only markers and returns cards to normal study eligibility

#### Scenario: Discard staged card
- **WHEN** the client calls `discard_staged_card` for a valid draft
- **THEN** the server deletes the staged note/cards and marks lifecycle state as `discarded`

#### Scenario: Prevent double-commit
- **WHEN** the client calls `commit_staged_card` on an already committed draft
...
- **WHEN** the staged note was modified in Anki GUI after draft creation and before `commit_staged_card`
...
- **THEN** the server rejects the request with `FORBIDDEN_OPERATION` and requires creating a new staged draft
...
- **WHEN** the client calls `create_staged_card` with `supersedesDraftId`
...
- **WHEN** `supersedesDraftId` points to a draft that is not `staged`
...
#### Scenario: List current staged drafts
- **WHEN** the client calls `list_staged_cards`
- **THEN** the server returns active drafts with `draftId`, creation timestamp, and current lifecycle state

#### Scenario: Cleanup old staged drafts
- **WHEN** the client calls `cleanup_staged_cards` with `olderThanHours`
...
- **WHEN** the client calls `cleanup_staged_cards` without `olderThanHours`
- **THEN** the server uses a default threshold of 72 hours

### Requirement: SQLite Persistence Contract for Draft Metadata
The MCP server MUST persist staged lifecycle metadata in SQLite with fixed table structures, indexes, and retention behavior.
...
- **WHEN** staged lifecycle operations run
...
- **THEN** `staged` and `superseded` metadata older than 72 hours is purged, while `committed` and `discarded` metadata is retained for 30 days for auditability
