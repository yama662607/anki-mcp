...
Implication:
- 破壊的操作（delete など）と review 完了操作（unsuspend）は分離し、呼び出し意図を明確化する。
...
Implication:
- 読み取り/検索/整形は高レベル統合、`add/update/delete/unsuspend` は分離が妥当。
...
Implication:
- GUI失敗時の recoverability は「既存 note を残したまま `update` / `delete` / `unsuspend` を選べること」で担保する。
...
- High-level (まとめる):
  - `list_decks`, `list_note_types`, `get_note_type_schema`, `search_notes`, `get_notes`, `open_note_preview`
- Fine-grained (分ける):
  - `add_note`, `update_note`, `delete_note`, `set_note_cards_suspended`

### B. Safety Policy

- study queue への露出は `set_note_cards_suspended(suspended=false)` のみ
- プレビュー段階では review-pending note を `update` または `delete` できる
- すべての状態遷移を構造化ログで記録
...
- 内部 idempotency メタデータの SQLite スキーマ進化
- マルチプロファイル時の internal metadata 分離
- GUI上の手動編集と `expectedModTimestamp` の衝突頻度
