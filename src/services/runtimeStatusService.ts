...
export class RuntimeStatusService {
  ...
  async getRuntimeStatus(input: { profileId?: string }): Promise<{
    ...
    if (capabilities.gatewayMode === 'memory') {
      guidance.push('Running in memory gateway mode. This is suitable for tests only, not for a real Anki collection.');
      ...
      guidance.push('Install anki-connect-extension if you want direct preview instead of edit-dialog fallback.');
    }
